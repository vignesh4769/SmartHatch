...
import { FiUserCheck, FiUserX, FiClock, FiCalendar, FiFilter, FiSave } from 'react-icons/fi';
import { useAuth } from '../../context/AuthContext';
import { getAttendanceByDate, submitAttendanceRecords, getEmployeesByHatchery } from '../../api/attendanceAPI';
...
function AttendanceManagement() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch employees
        const employeesResponse = await getEmployeesByHatchery(user.hatcheryName);
  ...
  const handleStatusChange = (employeeId, newStatus) => {
    setAttendanceRecords(prev => {
      const existingRecord = prev.find(record => record.employee === employeeId);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.success) {
        ...
        toast.success(`Successfully saved ${response.data.saved} attendance records`);
        
        // If there were any errors, show them
        if (response.data.errors && response.data.errors.length > 0) {
          toast.error(`Some records had errors: ${response.data.errors.join(', ')}`);
        }
  ...
  const getAttendanceStatus = (employeeId) => {
    const record = attendanceRecords.find(record => record.employee === employeeId);
                  ...
                  <tr key={employee._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleStatusChange(employee._id, 'late')}
                          className={`px-3 py-1 rounded-lg flex items-center ${
